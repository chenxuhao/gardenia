...
__global__ void bfs_kernel(int m, int *row_offsets, int *column_indices, foru *dist, bool *changed) {
...
void bfs(int m, int nnz, int *d_row_offsets, int *d_column_indices, unsigned *d_dist, int nSM) {
	...
	do {
		...
		printf("iteration=%d\n", iteration);
		bfs_kernel <<<nblocks, nthreads>>> (m, d_row_offsets, d_column_indices, d_dist, d_changed);

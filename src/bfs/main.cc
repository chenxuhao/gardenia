...
int main(int argc, char *argv[]) {
	...
	if (argc > 4) hub_factor = atof(argv[4]);
	...
	// distance array
#ifdef SIM
	DistT *h_dist = (DistT *) aligned_alloc(PAGE_SIZE, m * sizeof(DistT));
#else
	DistT *h_dist = (DistT *) malloc(m * sizeof(DistT));
#endif
	for(int i = 0; i < m; i ++) h_dist[i] = MYINFINITY;

	BFSSolver(m, nnz, source, in_row_offsets, in_column_indices, out_row_offsets, out_column_indices, in_degree, out_degree, h_dist);

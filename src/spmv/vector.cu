...
__global__ void spmv_csr_vector_kernel(const int num_rows, const int * Ap,  const int * Aj,
...
void SpmvSolver(int num_rows, int nnz, int *h_Ap, int *h_Aj, ValueType *h_Ax, ValueType *h_x, ValueType *h_y) { 
	...
	const unsigned int nthreads = BLOCK_SIZE;
	const unsigned int WARPS_PER_BLOCK = BLOCK_SIZE / WARP_SIZE;
	const unsigned int MAX_BLOCKS = MAX_THREADS / BLOCK_SIZE;
	const unsigned int nblocks = std::min(MAX_BLOCKS, DIVIDE_INTO(num_rows, WARPS_PER_BLOCK));
	printf("Start solving (nthreads=%d, nblocks=%d) ...\n", nthreads, nblocks);

	t.Start();
	//bind_x(d_x);
	spmv_csr_vector_kernel <<<nblocks, nthreads>>> (num_rows, d_Ap, d_Aj, d_Ax, d_x, d_y);   

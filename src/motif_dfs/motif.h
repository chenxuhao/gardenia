...
Kernel: Motif Couting
Author: Xuhao Chen

Will count the frequenncy of motifs (subgraphs/patterns/graphlets)
...
motif_omp : one thread per embedding using OpenMP
motif_base: one thread per embedding using CUDA
motif_warp: one warp per embedding using CUDA
*/
...
//void MotifVerifier(Graph &g, unsigned k, std::vector<uint64_t> test_total);

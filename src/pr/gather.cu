...
__global__ void process(int m, ScoreT *scores, int *degree, ScoreT *outgoing_contrib, bool *active) {
	...
	for (int src = tid; total_inputs > 0; src += blockDim.x * gridDim.x, total_inputs--) {
		...
		//if (src < m && active[src]) {
			...
			//if(src<3) printf("scores[%d]=%.8f, degree[%d]=%d\n", src, scores[src], src, degree[src]);
		}
	}
}
...
void PRSolver(int m, int nnz, int *h_row_offsets, int *h_column_indices, int *out_row_offsets, int *out_column_indices, int *h_degree, ScoreT *h_scores) {
	...
	//for(int i = 0; i < 5; i++) printf("scores[%d]=%.8f\n", i, h_scores[i]);
	...
	do {
		...
		process<<<nblocks, nthreads>>>(m, d_scores, d_degree, d_contrib, d_active);

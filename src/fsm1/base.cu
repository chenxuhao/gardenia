...
void FsmSolver(Graph &g, unsigned k, unsigned minsup, int nlabels, int &total_num) {
	...
	cuda_ctx.emb_list.init(nnz, k+1, false);
	...
	t.Start();
	unsigned level = 1;
	init_aggregate<<<nblocks, nthreads>>>(m, num_emb, cuda_ctx.gg, cuda_ctx.emb_list, pids, nlabels, minsup, small_sets, large_sets);
	...
	copy_vids<<<nblocks, nthreads>>>(num_emb, cuda_ctx.emb_list, vid_list0, vid_list1);
	CudaTest("solving copy_vids `failed");
	init_filter<<<nblocks, nthreads>>>(num_emb, cuda_ctx.emb_list, vid_list0, vid_list1, indices, is_frequent_emb);
	...
	cuda_ctx.emb_list.remove_tail(new_size);
	...
	while (1) {
		...
		aggregate_check<<<nblocks, nthreads>>>(num_emb, level, cuda_ctx.gg, cuda_ctx.emb_list, pids, nlabels, minsup, ne);
		...
		aggregate<<<nblocks, nthreads>>>(m, num_emb, level, cuda_ctx.gg, cuda_ctx.emb_list, pids, ne, id_map, nlabels, minsup, small_sets, middle_sets, large_sets);

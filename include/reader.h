...
inline DestID_** GenIndex(const pvector<SGOffset> &offsets, DestID_* neighs) {
  VertexID_ length = offsets.size();
...
//*/
template <typename VertexID_, typename DestID_ = VertexID_,
          typename WeightT_ = VertexID_, bool invert = true>
class Reader {
 ...
 public:
  ...
  EdgeList ReadInWEL(std::ifstream &in) {
    EdgeList el;
    VertexID_ u;
    VertexWeight<VertexID_, WeightT_> v;
    while (in >> u >> v) {
      el.push_back(Edge(u, v));
    }
    return el;
  ...
  EdgeList ReadInGR(std::ifstream &in) {
    ...
    while (!in.eof()) {
      ...
      if (c == 'a') {
        in >> c >> u >> v;
        el.push_back(Edge(u - 1, VertexWeight<VertexID_, WeightT_>(v.v-1, v.w)));
      } else {
        in.ignore(200, '\n');
      }
    }
  ...
  EdgeList ReadInMetis(std::ifstream &in, bool &needs_weights) {
    EdgeList el;
    VertexID_ num_vertices, num_edges;
    char c;
    std::string line;
    ...
    while (true) {
      ...
      if (c == '%') {
        ...
        header_stream >> num_vertices >> num_edges;
  ...
  //CSRGraph<VertexID_, DestID_, invert>& ReadSerializedGraph() {
  void ReadSerializedGraph(CSRGraph<VertexID_, DestID_, invert>& g) {
    bool weighted = GetSuffix() == ".wsg";
    if (!std::is_same<VertexID_, SGID>::value) {
      std::cout << "serialized graphs only allowed for 32bit" << std::endl;
      std::exit(-5);
    }
    if (!weighted && !std::is_same<VertexID_, DestID_>::value) {
    ...
    t.Start();
    bool directed;
    SGOffset num_vertices, num_edges;
    ...
    file.read(reinterpret_cast<char*>(&num_vertices), sizeof(SGOffset));
    pvector<SGOffset> offsets(num_vertices+1);
    neighs = new DestID_[num_edges];
    std::streamsize num_index_bytes = (num_vertices+1) * sizeof(SGOffset);
    ...
    if (directed && invert) {
      ...
      inv_index = GenIndex<VertexID_, DestID_>(offsets, inv_neighs);
      //inv_index = CSRGraph<VertexID_, DestID_>::GenIndex(offsets, inv_neighs);
    }
    file.close();
    t.Stop();
    PrintTime("Read Time", t.Seconds());
    if (directed)
      g.Setup(num_vertices, NULL, index, neighs, NULL, inv_index, inv_neighs);
      //return CSRGraph<VertexID_, DestID_, invert>(num_vertices, index, neighs, inv_index, inv_neighs);
    else
      g.Setup(num_vertices, NULL, index, neighs);
      //return CSRGraph<VertexID_, DestID_, invert>(num_vertices, index, neighs);
  }
};

...
//printf("hsize=%d\n", hsize);
	if (cudaMemcpy(items + hsize * sizeof(unsigned), copyfrom, nitems * sizeof(unsigned), cudaMemcpyHostToDevice) != cudaSuccess)
...
__device__ __forceinline__ unsigned Worklist::count() {
	if (*end >= *start) {
		return *end - *start;
	} else {
